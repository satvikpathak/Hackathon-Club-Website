    ...
    <div className="min-h-screen  text-white p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {hackathons.map((hackathon, index) => (
          ...
          >
            ...
            <p className="text-sm text-gray-400 mb-4">{hackathon.timeLeft}</p>
            <p className="text-lg mb-2">{hackathon.location}</p>
            <p className="text-lg font-bold mb-4">{hackathon.prize}</p>
            <p className="text-sm text-gray-400 mb-4">{hackathon.participants}</p>
            <div className="flex flex-wrap gap-2">
              {hackathon.tags.map((tag, i) => (
                <span key={i} className="bg-gray-700 text-sm px-3 py-1 rounded-full transition-transform transform hover:scale-110">
                  {tag}
                </span>
              ))}
            </div>
      ...
      {selectedHackathon && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 animate__animated animate__fadeIn">
          <div className="bg-gray-800 rounded-lg p-6 w-11/12 md:w-1/2 shadow-xl transform transition-transform duration-300 scale-100 hover:scale-105">
            <h2 className="text-3xl font-semibold mb-2">{selectedHackathon.title}</h2>
            <p className="text-sm text-gray-400 mb-4">{selectedHackathon.timeLeft}</p>
            <p className="text-lg mb-2">{selectedHackathon.location}</p>
            <p className="text-lg font-bold mb-4">{selectedHackathon.prize}</p>
            <p className="text-sm text-gray-400 mb-4">{selectedHackathon.participants}</p>
            <p className="text-sm text-gray-300 mb-4">{selectedHackathon.description}</p>
            <div className="flex justify-center">
              ...
              <button
                ...
                className="ml-4 bg-gray-700 text-white rounded-full py-2 px-4 transition-transform transform hover:scale-105"
              >
                Close
              </button>
            </div>
